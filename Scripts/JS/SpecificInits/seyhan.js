...
function updateArtwork() {
  ...
  if (bg) {
    dist =
      Math.abs(camera.position.x - bg.position.x) +
      Math.abs(camera.position.z - bg.position.z);
    var ezey = 0.01;
...
function updateCamera() {
  ...
  camera.position.z += (8.0 - camera.position.z) * 0.01;
  ...
  if (bg) camera.lookAt(bg.position);
